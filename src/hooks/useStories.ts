...
import { type LatestNews, type Category, type TopStories, type EditorPicks, MissedStories, FeaturedStory } from "../types/apiTypes";
        ...
        export const useEditorsPicks = () =>
          useQuery<EditorPicks[]>({
            ...
            queryFn: async () => {
              ...
              return res.data.data.data.map((item: any): EditorPicks => ({
                id: item.story.id,
                title: item.story.title,
                image: item.banner_image,
...
export const useSingleStory = (storyId : string ) =>
    useQuery({
        ...
        queryFn: async () => (await api.get(`/stories/${storyId}`))
    })
